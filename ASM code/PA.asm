...
# Question 2:
#$s0=5,$s1=5. Since I dont know what do the updated means, so i assume it run through a col-loop 
# Sice the update starts from the (5,4)to (5,6) the $s1 is actually still in 4 untill (5,6)
# is updated the score.After so, the pc goes to the colLoopMatchHorizontalContinue Lebal
# and then run the next tile (5,5) to check.so the $s1 is 5,if the updated means right after the 
#update of the score of the (5,6) tile, the $s1 will be 4 since the $s1 will be modified
#in the next label. the $s1 depends on the monment of when the hscore attribute of
#the tile at (5, 6) is updated
#--------------------------------------------------------------------
...
sw $s7, 0($sp)
	...
	la $t7,matchFound #assign the value zero to matchFound
    li $t4,0
    sw $t4,($t7)
	...
	li $t1,0 # col index=0
    	RowLoop:
		beq $t1,8,row_continue
		add $a0, $t0, $zero
        	add $a1, $t1, $zero
        	jal getCellAddress
        	add $t2, $v0, $zero #get [i][j] -> t2
...
row_continue:
	addi $t0,$t0,1 # if not, row++
	beq $t0,8,col_exit # if the row index is =8. End the loop
...
addi $sp, $sp, -32
	...
	#------ Your code starts here ------
	li $s0,7 # set row index =7
	row_loop_start:
		li $s1,0 # set col index=0
		beq $s0,-1,end_rowLoop
		col_loop_start:
			beq $s1,8,end_colloop
			
			add $a0,$s0,$zero
			add $a1,$s1,$zero
			jal getCellAddress
			add $t2,$v0,$zero # get back current tile address
			lw $t3,20($t2)# check is the match equal 0 if yes continue
			beq $t3,0,next_tile_col
			add $t3,$s0,$zero# prepare the index for searching the up one 
			Find_up:
			addi $t3,$t3,-1
			beq $t3,-1,next_tile_col
			
			add $a0,$t3,$zero
			add $a1,$s1,$zero
			jal getCellAddress
			add $t4,$v0,$zero # get the upper one address
			lw $t5,20($t4)
			bne $t5,0,Find_up  # if the upper one has match, keep going up
			#else swap the target($t0,$t1) ($t3,$t1)
			add $a0,$s0,$zero
			add $a1,$s1,$zero
			add $a2,$t3,$zero
			add $a3,$s1,$zero
...
addi $sp, $sp, -32
	...
	#
	#------ Your code starts here ------
	li $t1,0 #set the col index to 0
	col_loop_replace:
		li $t0,7 #set the row index =0
		beq $t1,8,end_col_loop_replace
		li $t7,-1# set every col basis multplier
		row_loop_replace:
			beq $t0,-1,end_row_loop_replace
			#get current tile address
			add $a0,$t0,$zero
			add $a1,$t1,$zero
			jal getCellAddress
			add $t2,$v0,$zero # get t2 as the address
			
			lw $t3,20($t2) # load the match if not equal to 0 do otherwise skip
			beq $t3,0,next_tile_row
			
			lw $t4,tileSize # get tilesize
			mult $t3,$t4
			mflo $t3 #get -$t7*tileSize
			sw $t3,4($t2) #save it into y 
			addi $t7,$t7,-1
			# end of step 2
			
			#start step 3
			lw $t3,score
			lw $t4,20($t2) # load match again 
			add $t3,$t3,$t4 # add them tgt
			
			la $t4,score
			sw $t3,($t4) #save score updated 
			
			li $t3,0
			sw $t3,20($t2)# save 0 into match , hscore and vscore 
			sw $t3,24($t2)
			sw $t3,28($t2)
			...
			li $v0,42
            		...
            		add $t3,$a0,$zero # this store the random number
            		sw $t3,16($t2) #assign a new random to kind 
            		#done for matched tile 
            		next_tile_row:
            		
            		addi $t0,$t0,-1# search up
            		j row_loop_replace
